var L = {
	zh: {
		...
		entropyWeak: "🔴 密码长度 {len} 字符，熵估算约 {bits} 位（弱）— 此类密码生成的私钥易被暴力破解，建议至少 128 位",
		entropyMid: "🟡 密码长度 {len} 字符，熵估算约 {bits} 位（一般）— 建议至少 128 位",
		entropyStrong: "🟢 密码长度 {len} 字符，熵估算约 {bits} 位（强）",
		lowEntropyAddrWarn: "⚠ 警告：以下地址由低熵密码（约 {bits} 位）生成，私钥易被暴力破解，请勿存入资金！",
		lowEntropyAddrWarnMid: "⚠ 注意：以下地址由中等强度密码（约 {bits} 位）生成，建议改用更高熵密码",
	...
	en: {
		...
		entropyWeak: "🔴 Password length: {len} chars, estimated entropy: ~{bits} bits (weak) — private keys from such passwords are easy to brute-force; aim for at least 128 bits",
		entropyMid: "🟡 Password length: {len} chars, estimated entropy: ~{bits} bits (fair) — aim for at least 128 bits",
		entropyStrong: "🟢 Password length: {len} chars, estimated entropy: ~{bits} bits (strong)",
		lowEntropyAddrWarn: "⚠ Warning: the following addresses were generated from a low-entropy password (~{bits} bits). The private keys are easy to brute-force; do not store funds on them!",
		lowEntropyAddrWarnMid: "⚠ Caution: the following addresses were generated from a fair-strength password (~{bits} bits). Consider using a higher-entropy password",
		...
		entropyMixing: "Mixing extra entropy...",
		entropyMixed: "Extra entropy mixed: ",
		...
		uncompTitle: "Uncompressed Format (Legacy)",
...
function setLang(lang) {
	...
	// Keep the passive entropy counter visible after language switch
	if (typeof entropyMixedBytes !== 'undefined' && entropyMixedBytes > 0) {
		var et = document.getElementById('entropy-text');
		if (et) et.textContent = t('entropyMixed') + ' ' + entropyMixedBytes + ' B';
	}
	// Refresh brain-wallet password entropy bar in the new language
	if (typeof updateBrainEntropy === 'function') updateBrainEntropy();
	// Refresh the low-entropy address warning (dynamic content) as well
	if (typeof updateLowEntropyAddrWarn === 'function') updateLowEntropyAddrWarn();
}
...
// Estimate password entropy in bits (charset size × length, then penalties
// for repeated runs, ascending/descending sequences, keyboard-row sequences
// and known-weak passwords). Warning only — never blocks generation.
var BRAIN_WEAK_PASSWORDS = ['password','123456','12345678','qwerty','abc123','111111','letmein','admin','welcome','monkey','dragon','master','iloveyou','sunshine','princess','football','passw0rd','000000','666666','888888'];

function estimatePasswordEntropy(pw) {
	if (!pw) return 0;
	var charset = 0;
	...
	var len = pw.length;
	var bits = len * Math.log2(charset);

	// Penalty: long runs of the same character (aaaa…)
	var runLen = 1, maxRun = 1;
	for (var i = 1; i < len; i++) {
		if (pw[i] === pw[i - 1]) { runLen++; if (runLen > maxRun) maxRun = runLen; }
		else runLen = 1;
	}
	if (maxRun >= 3) bits *= Math.pow(0.7, maxRun - 2);

	// Penalty: long ascending/descending sequences (abc, 321, …)
	var seqLen = 1, maxSeq = 1;
	for (var i = 1; i < len; i++) {
		var d = pw.charCodeAt(i) - pw.charCodeAt(i - 1);
		if (d === 1 || d === -1) { seqLen++; if (seqLen > maxSeq) maxSeq = seqLen; }
		else seqLen = 1;
	}
	if (maxSeq >= 4) bits *= 4 / maxSeq;

	// Penalty: keyboard-row sequences (qwerty, asdf, …)
	var rows = ['qwertyuiop', 'asdfghjkl', 'zxcvbnm', '1234567890', '!@#$%^&*()'];
	var low = pw.toLowerCase();
	for (var r = 0; r < rows.length; r++) {
		for (var i = 0; i + 4 <= rows[r].length; i++) {
			if (low.indexOf(rows[r].substr(i, 4)) !== -1) { bits *= 0.6; break; }
		}
	}
...
function updateBrainEntropy() {
	var bar = document.getElementById('brain-entropy-bar');
	if (!bar) return;
	var pw = document.getElementById('brain-pw').value;
	// Always show the estimate, including the empty password (0 bits):
	// an empty password can be submitted directly, so it must be flagged.
	var bits = estimatePasswordEntropy(pw);
	var len = pw.length;
	var cls, key;
	if (bits < 60) { cls = 'crypto-warn'; key = 'entropyWeak'; }
	else if (bits < 128) { cls = 'crypto-mid'; key = 'entropyMid'; }
	else { cls = 'crypto-ok'; key = 'entropyStrong'; }
	bar.className = 'crypto-status ' + cls;
	bar.textContent = t(key).replace('{len}', len).replace('{bits}', bits);
	bar.style.display = 'block';
}

// Re-render the low-entropy address warning in the current language.
// Only touches it while it is visible (set by showBrainResults).
function updateLowEntropyAddrWarn() {
	var warnEl = document.getElementById('brain-low-entropy-warn');
	if (!warnEl || warnEl.style.display !== 'block') return;
	var pwBits = estimatePasswordEntropy(document.getElementById('brain-pw').value || '');
	warnEl.className = 'crypto-status ' + (pwBits < 60 ? 'crypto-warn' : 'crypto-mid');
	warnEl.textContent = t(pwBits < 60 ? 'lowEntropyAddrWarn' : 'lowEntropyAddrWarnMid').replace('{bits}', pwBits);
}
...
function showBrainResults(entropy) {
	// Warn on addresses derived from low-entropy passwords (generation is
	// allowed, but the user must see the risk). Empty password ⇒ 0 bits.
	var pwBits = estimatePasswordEntropy(document.getElementById('brain-pw').value || '');
	var warnEl = document.getElementById('brain-low-entropy-warn');
	if (warnEl) {
		if (pwBits < 128) {
...
//
// Two values are mixed per event:
//   1. the absolute high-res timestamp (microsecond scale), and
//   2. the inter-event delta since the previous call — real scheduling
//      jitter between mouse/keyboard events. (Do NOT sample performance
//      .now() twice back-to-back: the samples are quantized and almost
//      always identical, so their XOR is useless.)
var _lastHighResNow = 0;
function seedHighResTiming() {
	if (typeof performance !== 'undefined' && typeof performance.now === 'function') {
		var t = Math.floor(performance.now() * 1000);
		SecureRandom.seedInt(t);
		var delta = (_lastHighResNow === 0) ? 0 : (t - _lastHighResNow) & 0xffffffff;
		if (delta !== 0) SecureRandom.seedInt(delta);
		_lastHighResNow = t;
	}
}

// Passive extra-entropy indicator: count how many bytes of mouse/keyboard/
// timing entropy have been mixed into the pool since page load, and show it
// next to the generate button (throttled DOM update, ~10 fps).
var entropyMixedBytes = 0, lastEntropyUiUpdate = 0;
(function () {
	var origSeedInt8 = SecureRandom.seedInt8;
	SecureRandom.seedInt8 = function (x) {
		origSeedInt8.call(SecureRandom, x);
		entropyMixedBytes++;
		var now = Date.now();
		if (now - lastEntropyUiUpdate > 100) {
			lastEntropyUiUpdate = now;
			var el = document.getElementById('entropy-text');
			if (el) el.textContent = t('entropyMixed') + ' ' + entropyMixedBytes + ' B';
		}
	};
})();
...
function generateRandom() {
	...
	// For a 12-word BIP39 mnemonic only the first 16 bytes are used as
	// mnemonic entropy (128 bits, per BIP39); 24 words use all 32 bytes.
...
var TESTS = {
	...
	wordlistSha256: 'AD90BF3BEB7B0EB7E5ACD74727DC0DA96E0A280A258354E7293FB7E211AC03DB'
};
...
function runSelfTest() {
	...
	try {
		// 校验值来自 BIP-39 官方词表: curl -s https://raw.githubusercontent.com/bitcoin/bips/master/bip-0039/english.txt | tr -d '\n\r' | sha256sum
		var ws = BIP39_WORDLIST_EN.join('');
		var wh = Crypto.SHA256(ws, { asBytes: true });
		var whHex = ''; for (var j = 0; j < wh.length; j++) whHex += ('0' + wh[j].toString(16)).slice(-2);
		check('BIP39 wordlist SHA256', whHex.toUpperCase(), TESTS.wordlistSha256);
	} catch(e) { testLog.push({ name: 'wordlist integrity', pass: false, expected: '', actual: e.message }); }
