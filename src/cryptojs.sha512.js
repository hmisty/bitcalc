(function () {
	...
	function wordsToBytes64(words) {
		var bytes = [];
		for (var i = 0; i < words.length; i++) {
			var w = words[i];
			for (var j = 7; j >= 0; j--) {
				bytes.push(Number((w >> BigInt(j * 8)) & 0xffn));
			}
		}
		return bytes;
	}
	...
	var SHA512 = C.SHA512 = function (message, options) {
		if (message.constructor == String) message = C.charenc.UTF8.stringToBytes(message);

		var m = message.slice();
		var origLenBits = m.length * 8;

		m.push(0x80);
		while ((m.length * 8) % 1024 !== 896) {
			m.push(0x00);
		}

		var lenBytes = [];
		for (var i = 7; i >= 0; i--) {
			lenBytes.push(Math.floor(origLenBits / Math.pow(256, i + 8)) % 256);
		}
		for (var i = 7; i >= 0; i--) {
			lenBytes.push(Math.floor(origLenBits / Math.pow(256, i)) % 256);
		}
		m = m.concat(lenBytes);
		...
		for (var offset = 0; offset < m.length; offset += 128) {
			var blockBytes = m.slice(offset, offset + 128);
			var W = bytesToWords64(blockBytes);
			W.length = 80;

			for (var t = 16; t < 80; t++) {
				var s0 = ror64(W[t - 15], 1n) ^ ror64(W[t - 15], 8n) ^ (W[t - 15] >> 7n);
				var s1 = ror64(W[t - 2], 19n) ^ ror64(W[t - 2], 61n) ^ (W[t - 2] >> 6n);
			...
			for (var t = 0; t < 80; t++) {
				var S1 = ror64(e, 14n) ^ ror64(e, 18n) ^ ror64(e, 41n);
				var ch = (e & f) ^ ((~e) & g);
				var temp1 = (h + S1 + ch + K[t] + W[t]) & 0xffffffffffffffffn;
				var S0 = ror64(a, 28n) ^ ror64(a, 34n) ^ ror64(a, 39n);
				var maj = (a & b) ^ (a & c) ^ (b & c);
		...
		var digestbytes = wordsToBytes64(H);

		return options && options.asBytes ? digestbytes :
			options && options.asString ? C.charenc.Binary.bytesToString(digestbytes) :
			util.bytesToHex(digestbytes);
	};

	SHA512._blocksize = 32;
	SHA512._digestsize = 64;
