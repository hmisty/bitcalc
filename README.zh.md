...
| 功能 | 说明 |
|------|------|
| **脑钱包** | 密码 → SHA256ⁿ → 熵，可选迭代次数；实时密码熵评估与低熵警告（不阻止生成） |
...
| **加载动画** | 计算时显示旋转动画遮罩 |
| **QR Code** | 地址和私钥旁显示二维码 |
| **敏感信息遮罩** | 助记词、WIF 私钥、熵/种子 hex 默认模糊遮罩，点击后显示（防肩窥） |
| **移动端适配** | 视口驱动的单列布局，触控目标加大 |
| **深色模式** | 自动跟随系统浅色/深色主题（`prefers-color-scheme`，零 JS） |
| **中英双语** | 中文 / English 界面切换 |
| **完全离线** | 单 HTML 文件，浏览器直接打开 |

## 更新日志

### 2026-08-03 — 深色模式（跟随系统主题）

- **跟随系统浅色/深色**: 全部颜色收敛为 CSS 自定义属性；`@media (prefers-color-scheme: dark)` 随系统深浅色自动切换整个页面——无开关、无 JS、无需刷新
- **深色调色板**: 深藏青底色（页面 `#12141d` / 卡片 `#1b1f2b`），与恒深色页头同族；保留比特币橙强调色；状态横幅、crypto 指示框、表单控件、加载遮罩均有深色变体
- **主题自适应二维码**: QR canvas 绘制时读取 `--qr-fg`/`--qr-bg` CSS 变量——深色模式下浅色模块 + 深色底
- **内联样式清理**: 模板中约 20 处内联颜色样式改为变量；按主题设置 `color-scheme`（原生滚动条/表单控件跟随）；新增 `theme-color` meta 适配移动端浏览器外观

### 2026-08-02 — UI 打磨与移动端支持

- **BC 徽章**: Logo 恢复为「BitCalc」并附橙色「BC」简称徽章；副标题不再重复品牌名，窄屏时自动换行到 logo 下方（PC 端保持右侧）
- **敏感信息遮罩**: 助记词、WIF 私钥、熵与种子 hex（共 12 个字段）默认模糊遮罩，点击后显示——防肩窥保护（值仍在 DOM 中，非加密）
- **移动端支持**: 新增 viewport meta（手机端显示过小的根因——此前页面按 980px 布局再缩小）；窄屏切换单列布局：tabs 均分整行、表单纵向堆叠、QR 单列、按钮全宽；输入框 16px 同时防止 iOS 自动缩放
- **页脚**: 新增使用建议（下载后离线 + 无痕模式打开）；新增版本行「当前版本 2026.08.02」（以发布日期为版本号）；捐赠区移至分隔线下独立区块并加 ☕ 图标
...
- **脑钱包密码熵评估器**: 密码输入框上方实时显示熵估算（字符集 × 长度，扣除模式惩罚：重复串、升降序序列、键盘行序列、常见弱密码黑名单）。同时显示密码长度与熵位数，评级为 <60 位弱 / 60-127 位一般 / ≥128 位强；空密码立即提示
- **低熵地址警告**: 生成后，若地址由 <128 位密码派生，输出区顶部显示红色/橙色警告横幅（仅警告——生成从不被阻止）。短于 30 字符的密码（与复杂度提示一致）永远不能评为强
